...
cryptor = Cryptor.new(:aes_192_ctr, 24)
block_size = 16
...
chosen_clear_text = 'j' * ((prefix_length / block_size + 1) * block_size - prefix_length)
puts "chosen clear text: #{chosen_clear_text} (#{chosen_clear_text.length})"
...
injection_range = ((prefix_length + chosen_clear_text.length - 1 * block_size)..(prefix_length + chosen_clear_text.length - 1))
...
altered_block = cipher_text[injection_range] ^ xor_mask
print "altered_block:"
pp altered_block

altered_cipher_text = cipher_text[0..(prefix_length + chosen_clear_text.length - block_size - 1)] + altered_block + cipher_text[(prefix_length + chosen_clear_text.length)..-1]
