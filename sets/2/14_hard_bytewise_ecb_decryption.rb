...
cryptor = Cryptor.new(:aes_192_cbc, 24, base64_string.base64_to_byte_string, "\x00" * 16)
...
secret_length.times do |i|
  ...
  target = cryptor.encrypt(base_clear_text)[0..length]

  (0..255).each do |n|
    char = n.chr
    potential_match = cryptor.encrypt(base_clear_text + secret_string + char)[0..length]
